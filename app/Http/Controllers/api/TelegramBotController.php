...
class TelegramBotController extends Controller
{
    ...
    protected function _init() {
        ...
        $this->supportAkk = Setting::getUrl(Setting::getValByName('supportAkk'));
 ...
 //               $this->_start($userId);
    ...
    protected function _registration($user, $text)
    {
        try {
            ...
            $model->is_referer_bonus_payout = 0;
                ...
                if (!empty($model->referer_id)) {
                    ...
                    if (!is_null($referer)) {
                        $validator = Validator::make([
                            ...
                            'user_id' => Rule::unique('referals')
                                ->where('referal_id', $model->referal_id),
                        ]);
                        if (!$validator->fails()) {
                            $referal = new Referal;
...
//                        ];
                        ...
                        $referalsInfo = $this->_myReferals($user);
                        $message = $referalsInfo['message'];
                        $buttons = $referalsInfo['buttons'];
                    ...
                    } elseif (mb_strpos($text, 'Вывод денег') !== false) {
                        $userInfo = User1::getInfo($user);
                    ...
                    } elseif (mb_strpos($text, 'Поддержка') !== false) {
                        $message = "Перейдите по ссылке ниже: \xE2\xAC\x87";
                        $buttons = [
                            [
                                ['text' => "Поддержка", 'callback_data' => 'support', 'url' => $this->supportAkk],
                            ],
                        ];
...
//                ],
    ...
    protected function _myReferals($user) {
        $refererNickname = '';
